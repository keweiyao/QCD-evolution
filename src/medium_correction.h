...
/* Tables: Rqq, Dqq, Aqg,
           ...
           Rgg, Dgg, Agg,
                     Agq, Agc, Agb;
// 3D tables: R and A[X=lnE, Y=lnkt, Z=ln(z/(1-z))], 
//            NX=10, NY=101, NZ=101
...
public:
    ...
    double Get(std::string channel, std::vector<double> input){ // e.g. Type=R, a=q, b=q, input=(E, kT2 or Q2, z)
        // check if this channel exists:
        if (elem_exist<std::string>(channels_, channel)){
            int TableDim = Tables[channel].dim();
            std::vector<double> transformed_input;
            transformed_input.resize(TableDim);
